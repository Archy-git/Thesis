...
from schedule import TEST_DURATION
...
#   L3  hop (TOTP),  dual radio, BL+detect, with jamming schedule (sync)
#   L4  hop (TOTP), single radio (retune at boundary), no BL, no detect, no jam
#   L5  hop (TOTP),  dual radio, no BL, no detect, no jam
#   L6  hop (TOTP),  dual radio, BL+detect, with jamming schedule (per-node!)
USE_BL          = MODE in ("L3", "L6")
DETECT_ENABLED  = MODE in ("L3", "L6")
DUAL_RADIO      = MODE in ("L2", "L3", "L5", "L6")
STATIC          = (MODE == "L1")
...
class ArtchoNode:
    ...
    def tune_iface(iface, freq, retries=2):
        ...
        Idempotent: if the iface is already on the target channel, this is a
        no-op. We DO NOT leave-and-rejoin the IBSS in that case, because the
        leave/join cycle disrupts the cell and causes a multi-second warm-up
        on the link.
        """
        ...
        # Idempotent fast-path: already on the right channel -> nothing to do.
        if ArtchoNode.read_iface_channel(iface) == expected_ch:
            return True
    ...
    def _pre_warm(self):
        """Tune the radio(s) to where they need to be AT TEST_EPOCH, BEFORE
        the wait for TEST_EPOCH. This lets the IBSS cell stabilize on the
        first test bucket's channel during the ~20s setup window, so we
        don't lose ~50% of packets to cell migration in the first few
        seconds of measurement. tune_iface is idempotent, so cold_start's
        later call is a no-op when channels are already correct.
        """
        # What bucket will we be in at TEST_EPOCH?
        if TEST_EPOCH > 0 and BUCKET_EPOCH > 0:
            target_bucket = int((TEST_EPOCH - BUCKET_EPOCH) / BUCKET_SECONDS)
        else:
            target_bucket = compute_bucket()

        if STATIC:
            freq = CH_TO_FREQ[STATIC_CHANNEL]
            print(f"[PRE-WARM] L1 static -> wlan0 ch{STATIC_CHANNEL}")
            self.tune_iface("wlan0", freq)
            return

        bl = self.read_blacklist_file() if USE_BL else {}
        cur_freq, cur_ch = self.compute_totp(target_bucket, bl)

        if not DUAL_RADIO:
            # L4: single radio, retune-at-boundary
            print(f"[PRE-WARM] L4 -> wlan0 ch{cur_ch} (first test bucket = {target_bucket})")
            self.tune_iface("wlan0", cur_freq)
            return

        # Dual radio (L2, L3, L5): active = first bucket, bg = first+1
        nxt_freq, nxt_ch = self.compute_totp(target_bucket + 1, bl)
        active, bg = self.role_for_bucket(target_bucket)
        print(f"[PRE-WARM] {MODE} -> {active} ch{cur_ch} (active, bucket {target_bucket}), "
              f"{bg} ch{nxt_ch} (bg, bucket {target_bucket+1})")
    ...
    def run(self):
        ...
        # Defer radio reconfiguration so the SSH session that launched us
        # has time to close cleanly. The management plane (SSH from Node A)
        # rides on the SAME IBSS link the radios are about to be retuned
        # on; pre_warm's `iw ibss leave/join` will tear down the cell and
        # break the in-flight SSH ack ("B-LAUNCHED"), causing testdash to
        # time out its launch command and orphan us. 5s is plenty —
        # testdash's 45s buffer absorbs this easily.
        if TEST_EPOCH > 0 and TEST_EPOCH - time.time() > 10:
            print("[STARTUP] sleeping 5s to let launch SSH close before pre_warm...")
            time.sleep(5.0)

        # Pre-warm the radios to the FIRST test bucket's channel(s) BEFORE
        # we wait for TEST_EPOCH. The IBSS cell needs ~1-5s to migrate to
        # a new channel; doing this during the wait window means the cell
        # is stable by the time testworker starts firing packets at
        # TEST_EPOCH, so we don't bleed ~50% PDR in the first seconds.
        self._pre_warm()
        ...
        if TEST_EPOCH > 0:
            ...
            if wait_s > 0:
                print(f"[WAIT] sleeping {wait_s:.1f}s until TEST_EPOCH (radios pre-warmed)...")
                time.sleep(wait_s)
            elif wait_s < -1.0:
                # We started this process AFTER TEST_EPOCH already passed.
                # Whatever's happening upstream let the buffer run out;
                # this run's first-bucket data will be unreliable.
                print(f"[LATE START] launched {-wait_s:.2f}s past TEST_EPOCH — "
                      f"first bucket(s) may be lost while IBSS cell settles.")
        ...
        # If TEST_EPOCH is set, exit after the test duration is done (+grace).
        # Prevents orphaned artcho processes from hopping forever if testdash
        # fails to kill us — which would tear down the management plane on
        # every bucket boundary and break all subsequent windows.
        exit_at = TEST_EPOCH + TEST_DURATION + 15.0 if TEST_EPOCH > 0 else None
        ...
        while True:
            ...
            if exit_at is not None and now > exit_at:
                print(f"[EXIT] test duration ({TEST_DURATION}s) + 15s grace elapsed, "
                      f"exiting cleanly so we don't orphan the management plane.")
                sys.exit(0)
