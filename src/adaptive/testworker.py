...
#   L6 dual+BL+detect (per-node jamming) — behaves like L3 on this side
...
def jammer_loop():
    ...
    while True:
        ...
        jammed_chs = get_jammed_channels(test_t, MODE, NODE_ID)
        for iface in INTERFACES:
    ...
    while True:
        try:
            ...
            status = {
                ...
                "jammed_now":      get_jammed_channels(test_t, MODE, NODE_ID) if test_t >= 0 else [],
                "transport":       TRANSPORT,
                "dest_addr":       DEST_ADDR,
